...
export default function Page({params}) {
    ...
    const [movie, setMovie] = useState<{original_title: string, budget: number, overview: string, poster_path: string, release_date: string, revenue: number, runtime: number, genres: {id: number, name: string}[], production_companies: {id: number, logo_path: string, name: string}[]}>({});
    ...
    const {original_title, backdrop_path, budget, overview, poster_path, release_date, revenue, runtime, vote_average, genres, production_companies} = movie;
    ...
    return (
      <>
        ...
        <h1 className='container text-3xl text-white mb-6 font-medium'>{original_title ? `${original_title}` : ""}</h1>
